...
from lead_finder import find_business_leads, format_leads_message
...
(SETUP_EMAIL, SETUP_PASSWORD, ADD_LEAD_EMAIL, ADD_LEAD_FIRST_NAME, ADD_LEAD_COMPANY,
 ...
 AI_SUBJECTS_BUSINESS, AI_SUBJECTS_AUDIENCE, LEADS_SEARCH_INPUT) = range(35)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔍 Lead Finder", callback_data="menu_leadfinder")]
    ]
    welcome = (
        "🚀 *Welcome to Cold Email Outreach Bot!*\n\n"
        "Manage leads, send cold emails, track deals & grow your business.\n"
        "🤖 AI-powered email writer & 🔍 Lead Finder!\n\n"
...
async def dashboard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "menu_leadfinder":
        keyboard = [
            [InlineKeyboardButton("🔍 Find Leads", callback_data="cmd_leads")],
            back_btn
        ]
        await query.edit_message_text(
            "🔍 *Business Lead Finder*\n\n"
            "Find real business leads with contact info!\n\n"
            "🎯 Enter a niche + location (e.g., 'dentists in Kakinada')\n"
            "📧 Get emails, phone numbers & websites\n"
            "💾 Save leads directly to your list\n\n"
            "Type /leads to start searching!",
            parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
...
🔍 *Lead Finder*
/leads - Find business leads by niche & location
...
# ===================== BUSINESS LEAD FINDER =====================
async def leads_search_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🔍 *Business Lead Finder*\n\n"
        "Enter a niche and location to find business leads.\n\n"
        "*Format:* `niche in location`\n\n"
        "*Examples:*\n"
        "• dentists in Kakinada\n"
    ...
    return LEADS_SEARCH_INPUT

async def leads_search_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.message.text.strip()
    
    # Parse niche and location
    niche = query
    location = ""
    ...
    if not location:
        await update.message.reply_text(
            "⚠️ Please include a location.\n\n"
            "Format: `niche in location`\n"
            "Example: `dentists in Kakinada`\n\n"
            "Try again or /cancel:",
            parse_mode='Markdown'
        )
        return LEADS_SEARCH_INPUT
    ...
    leads = await loop.run_in_executor(None, find_business_leads, niche, location, 10)
    
    # Format and send results
    message = format_leads_message(leads, niche, location)
    ...
    # Offer to save leads
    if leads:
        # Store leads in context for saving
        context.user_data['found_leads'] = leads
        keyboard = [
            [InlineKeyboardButton("💾 Save All Leads", callback_data="save_found_leads")],
            [InlineKeyboardButton("🔍 Search Again", callback_data="cmd_leads")],
            [InlineKeyboardButton("🔙 Main Menu", callback_data="menu_main")]
        ]
        await update.message.reply_text(
            "Would you like to save these leads to your list?",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
    
    return ConversationHandler.END

async def save_found_leads_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    
    leads = context.user_data.get('found_leads', [])
    if not leads:
        await query.edit_message_text("❌ No leads to save. Use /leads to search first.")
        return
    ...
    try:
        for lead in leads:
            ...
            if emails:
                for email in emails:
                    ...
                    existing = db.query(Lead).filter_by(user_id=user.id, email=email).first()
                    if not existing:
                        new_lead = Lead(
                            user_id=user.id,
                            email=email,
                            first_name=lead.get('business_name', '')[:50],
        ...
        await query.edit_message_text(
            f"✅ *Leads Saved!*\n\n"
            f"💾 Saved: {saved} leads\n"
            f"⏭ Skipped: {skipped} (no email or duplicate)\n\n"
            f"Use /viewleads to see all your leads.\n"
            f"Use /campaign to email them!",
            parse_mode='Markdown'
        )
    except Exception as e:
        db.rollback()
        await query.edit_message_text(f"❌ Error saving leads: {str(e)}")
    finally:
        db.close()
    
    context.user_data.pop('found_leads', None)
...
if __name__ == "__main__":
    ...
    # Lead Finder conversation handler
    app.add_handler(ConversationHandler(
        entry_points=[CommandHandler('leads', leads_search_start)],
        states={
            LEADS_SEARCH_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, leads_search_process)],
        },
        fallbacks=[CommandHandler('cancel', cancel)]))

    # Save found leads callback
    app.add_handler(CallbackQueryHandler(save_found_leads_callback, pattern='^save_found_leads$'))
